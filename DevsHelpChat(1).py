...
version = f"v{__version__[0]}.{__version__[1]}.{__version__[2]}alpha_release"
ver = f"<i>DevsHelpChat {version}</i>"
...
class HikariChatAPI:
    ...
    async def init(
        ...
        module: "DevsHelpChatMod",  # type: ignore
    ):
        """Entry point"""
        self._client = client
        self._db = db
    ...
    async def _wss(self):
        async with websockets.connect(
            ...
            logger.debug(f"DevsHelpChat connection debug info {init}")
            ...
            logger.debug("DevsHelpChat connected")
            self._show_warning = True
    ...
    async def _connect(self):
        while True:
            try:
                ...
                logger.debug("DevsHelpChat disconnection traceback", exc_info=True)
                ...
                if self._show_warning:
                    logger.debug("DevsHelpChat disconnected, retry in 5 sec")
...
@loader.tds
class DevsHelpChatMod(loader.Module):
    """
    ...
    strings = {
        ...
        "version": (
            "<b>🪆 {}</b>\n\n"
            "<b>😌 Author: @Vadym_Yem | Anonymous</b>\n"
            "<b>📥 Downloaded from @Vadym_Yem</b>\n"
            "<b>{}</b>"
        ),
        "error": "😵 <b>DevsHelpChat Issued error</b>",
        ...
        "banninja_report": (
            ...
            "<i>Deleted {} bot(-s)</i>\n\n"
            "🏹 <i>«BanNinja can handle any size of attack»</i> © <code>@Vadym_Yem</code>"
        ),
        "confirm_rmfed": (
    ...
    strings_ru = {
        ...
        "error": "😵 <b>Произошла ошибка DevsHelpChat</b>",
        ...
        "api_error": "🚫 <b>Ошибка DevsHelpChat!</b>\n<code>{}</code>",
    ...
    def error_handler(func) -> FunctionType:
        ...
        async def wrapped(*args, **kwargs):
            try:
                ...
                logger.exception("Exception caught in DevsHelpChat")
    ...
    async def punish(
        ...
        """
        self._punish_queue += [[chat_id, user, violation, action, user_name, fulltime]]
    ...
    async def p__banninja(
        ...
        if len(self._join_ratelimit[chat_id]) >= self.config["join_ratelimit"]:
            ...
            for u, m in self._ban_ninja_messages:
                if u not in users:
                    if len(users) > self.confug["join_ratelimit"]:
                        break
    ...
    async def _global_queue_handler_process(self, message: Message):
        ...
        if cas_result:
            await self.punish(chat_id, user, "cas", r, user_name)
            return
